...
- War Factory production now includes imported Bastion light, medium, and heavy tank silhouettes. Barracks/Command Center production now includes imported Bastion gunner, grenadier, rocket soldier, flame trooper, and engineer infantry silhouettes. Grenadiers/flamers add splash damage, rocket soldiers deal bonus damage to armored targets, engineers repair damaged friendly units or structures, and the medium tank can board one infantry passenger, saves/restores that passenger type/count, and fires the passenger weapon while attacking.
...
- EditMode tests: `101` total, `101` passed, `0` failed.
