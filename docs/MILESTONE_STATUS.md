...
- Desktop mode now presents a main menu before play and a pause menu during skirmish play, with start/resume, restart, save, load, and quit actions.
...
- Quest mode uses a compact world-space status panel for credits, power, selected count, status text, core command hints, and the X/left-primary command-console hint, with cached refreshes that update only when displayed state changes. The Quest tactical map shows a non-interactive battle-view panel with pooled resource and visible-force pips. The Quest command console is now a left-wrist build menu with a layered holographic frame, large color-coded structure/unit selection tiles, and text labels.
- War Factory production now includes imported Bastion light, medium, and heavy tank silhouettes. Barracks/Command Center production now includes imported Bastion gunner, grenadier, rocket soldier, flame trooper, and engineer infantry silhouettes. Base construction now uses the imported Bastion fabrication hub for the command center plus Bastion power plant, barracks, refinery, war factory, harvester, turret, gun tower, and advanced gun tower silhouettes. Imported units and buildings now get extra team-color roof/side plates plus textured armor/detail panels so skirmish teams and silhouettes read more clearly. Grenadiers/flamers add splash damage, rocket soldiers deal bonus damage to armored targets, engineers repair damaged friendly units or structures, and the medium tank can board one infantry passenger, saves/restores that passenger type/count, and fires the passenger weapon while attacking.
- Armed combat units now auto-acquire nearby enemies while idle and retaliate against attackers, making skirmish fights less dependent on constant manual attack orders.
...
- The generated battlefield includes original primitive set dressing for a tabletop war-room read: procedurally textured sand, darker dune shelves, projected water ripples, rocky ridges, transparent scorch marks, rails, and corner pylons. The set dressing is visual-only so it does not steal command raycasts from the battlefield.

## Architecture

- `RtsRuntimeModeResolver` chooses `Desktop` or `QuestVr` from forced test settings, command-line/environment overrides, active XR device state, or an initialized Android OpenXR loader for Quest builds. EditMode tests cover override precedence, automatic XR fallback, and the Android path staying desktop when no OpenXR loader is active.
...
- EditMode tests: `102` total, `102` passed, `0` failed.
...
- Screenshot exporter: produces a populated desktop-board showcase at `C:\Users\matth\Documents\Codex\2026-06-24\i-s\outputs\quest-command-rts-sample.png`, a Quest-view world-space UI showcase at `C:\Users\matth\Documents\Codex\2026-06-24\i-s\outputs\quest-command-rts-quest-sample.png`, a room-sized Quest profile showcase at `C:\Users\matth\Documents\Codex\2026-06-24\i-s\outputs\quest-command-rts-quest-room-sample.png`, and a fog-hidden terrain texture close-up at `C:\Users\matth\Documents\Codex\2026-06-24\i-s\outputs\quest-command-rts-ground-texture-sample.png`.
